...
describe('foundation hooks and field semantics', () => {
  ...
  it('merges object and callback refs', () => {
    const objectRef = createRef<HTMLInputElement>();
    const callback = vi.fn();
    function Probe() { const ref = useMergedRefs(objectRef, callback); return <input ref={ref} />; }
  ...
  it('reacts to direction changes and keeps portals SSR-safe', async () => {
    const ref = createRef<HTMLDivElement>();
    const { result } = renderHook(() => useDirection(ref));
    render(<div ref={ref} dir="rtl" />);
  ...
  it('resets uncontrolled text through the native form reset event', async () => {
    function Form() { const [last, setLast] = useState(''); return <form><CgField label="Name"><CgTextBox defaultValue="CashGear" onValueChange={setLast} /></CgField><output>{last}</output><button type="reset">Reset</button></form>; }
    render(<Form />);
