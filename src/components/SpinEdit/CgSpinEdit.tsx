...
export const CgSpinEdit = forwardRef<HTMLInputElement, CgSpinEditProps>(function CgSpinEdit(
  ...
  const stepBy = (direction: 1 | -1, amount = step) => {
    ...
    change(normalizeNumericValue(raw, min, max, precision, roundingMode), 'step');
  };
  ...
  return <CgNumericEditCore {...props} roundingMode={roundingMode} ref={mergedRef} value={current} defaultValue={defaultValue} locale={locale} formatStyle={formatStyle} currency={currency} useGrouping={useGrouping} min={min} max={max} precision={precision} step={step} disabled={disabled} readOnly={readOnly} buttons={buttons} classifyDraft={classifyDraft} inputMode={inputMode ?? (allowExpressions ? 'text' : 'decimal')} updateValueOnInput={updateValueOnInput} suppressDuplicateCommit={updateValueOnInput} role={enableKeyboardStepping ? 'spinbutton' : 'textbox'} aria-valuemin={enableKeyboardStepping ? min : undefined} aria-valuemax={enableKeyboardStepping ? max : undefined} aria-valuenow={enableKeyboardStepping ? current ?? undefined : undefined} onChange={(event) => { draftRef.current = event.target.value; onChange?.(event); }} onValueChange={(next, details) => { if (details.reason !== 'input') draftRef.current = ''; currentRef.current = next; setCurrent(next); return onValueChange?.(next, details); }} onKeyDown={(event) => { if (!enableKeyboardStepping) { onKeyDown?.(event); return; } if (event.key === 'ArrowUp') { event.preventDefault(); stepBy(1); } else if (event.key === 'ArrowDown') { event.preventDefault(); stepBy(-1); } else if (pageStep && event.key === 'PageUp') { event.preventDefault(); stepBy(1, pageStep); } else if (pageStep && event.key === 'PageDown') { event.preventDefault(); stepBy(-1, pageStep); } onKeyDown?.(event); }} />;
});
