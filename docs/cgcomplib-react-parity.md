# CashGear Blazor UI → `@cashgear/ui` Phase 1–25 parity
...
| `CgComboBox` | Mirrored | `CG.CompLib/Comp/Inputs/CgComboBox.*`; `CG.CompLib.Demo/Components/Pages/ComboBoxDemo.razor`; reliability refresh from `CashGear.Blazor.UI@c5c03fb9` | `src/components/ComboBox/*`, `src/internal/PositionedOverlay.tsx` | `tests/combo-box.test.tsx`, Phase 24.1 browser coverage | `src/components/ComboBox/CgComboBox.stories.tsx`, Phase 24.1 reliability stories | Object-valued selection uses stable keys for identity/form serialization. Query context and data version invalidate stale work; incomplete results require explicit keyboard highlighting; diagnostic errors remain separate from safe rendered copy. |
| `CgKeyComboBox` | Mirrored | `CG.CompLib/Comp/Inputs/CgKeyComboBox.*`; `CG.CompLib.Demo/Components/Pages/Home.razor`; reliability refresh from `CashGear.Blazor.UI@c5c03fb9` | `src/components/KeyComboBox/*` | `tests/key-combo-box.test.tsx`, Phase 24.1 browser coverage | `src/components/KeyComboBox/CgKeyComboBox.stories.tsx`, Phase 24.1 reliability stories | React keys are strings or finite numbers, `null` is empty, and `selectedItem` has synchronous precedence. Abortable cached resolution, mismatch rejection, safe key fallback, diagnostics, and explicit refresh mirror the refreshed source. |
| `CgLookUpGrid` | Mirrored with focused lookup descriptors | `CG.CompLib/Comp/Inputs/CgLookUpGrid.*`; `CgLookUpGrid*Tests.cs`; `LookUpGridDemo.razor`; `LookUpGridErpDemo.razor`; reliability refresh from `CashGear.Blazor.UI@c5c03fb9` | `src/components/LookUpGrid/*` | `tests/look-up-grid.test.tsx`, Phase 24.1 browser coverage | `src/components/LookUpGrid/CgLookUpGrid.stories.tsx`, Phase 24.1 reliability stories | Immutable descriptors, AbortSignal loaders/resolvers, `null`, caller-owned query-context equality, data-version invalidation, diagnostic callbacks, selected-item refresh, and a native form proxy replace registration, cancellation tokens, and EditContext. No virtualization prop is exposed. |
...
| `CgPdfViewer` | Mirrored with typed source and asset deployment adaptations | `CashGear.Blazor.UI@32edd96e1220ec5cf1b5ed691795fd95f605c19a: Components/Documents/PdfViewer/*` | `src/components/PdfViewer/*`, `src/vendor/pdfjs/*` | `tests/pdf-viewer-model.test.ts`, `tests/pdf-viewer.test.tsx`, Phase 25 browser semantic/Axe/visual coverage | `src/components/PdfViewer/CgPdfViewer.stories.tsx` | A URL/data/provider union, `AbortSignal` generations, render callbacks, and an actions ref replace .NET streams, `EventCallback`, Razor fragments, and JS interop handles. The same PDF.js runtime is pinned and packaged locally; links and forms remain opt-in. |
...
## Phase 24.1 — Grid and Lookup Reliability Refresh

Reference audit: this phase mirrors the relevant clean `CashGear.Blazor.UI` state at commit `c5c03fb9e5bc2f49b7a3af87729cfbb83ab86c1c`. Unrelated application work was not inspected or modified. React retains immutable descriptors, controlled-state authority, native DOM/form semantics, `AbortSignal`, and the existing surface/text token system.

| Area | React implementation | Source-aligned behavior and adaptation |
| --- | --- | --- |
| Grid appearance/state | `src/components/Grid/state.ts`, `CgGrid.tsx`, `CgGrid.module.css`, private `TableStylePicker.*` | State v11 persists nullable appearance; v1–v10 migrate to `null`. The Grid-integrated popup provides 18 palette presets, logical banding/borders, row density, header/edge/total emphasis, label overrides, saved-view hints, normalized controlled proposals, and reset-to-initial behavior without provider reload. `null` preserves legacy styling and striping. React reuses its own surface/text tokens and adds no virtualization. |
| Grid editing | `src/components/Grid/editing.tsx`, `CgGrid.tsx`, `CgGrid.types.ts` | Raw numeric/date/select drafts are tracked separately from valid models. Metadata rules and synchronous cross-field `validateEdit` merge across popup, inline-row, cell, and atomic batch modes. Pending mutations disable and inert complete editor surfaces, reject competing operations, ignore stale callbacks, retain drafts on failure/cancellation, and never expose thrown persistence details. |
| ComboBox | `src/components/ComboBox/*` | Generic query context and `dataVersion` invalidate requests/local indexes. Safe diagnostics expose raw errors to callbacks only. Local truncation and remote page fullness mark results incomplete; Enter then requires explicit highlight. IME composition suppresses commit/navigation and clearable editors expose Ctrl+Backspace/Delete. |
| KeyComboBox | `src/components/KeyComboBox/*` | `selectedItem` remains the synchronous authority. Abortable resolution caches success, missing values, and failures by key/context/version; mismatched keys fail safely. Bound keys retain string/form fallbacks and `refreshSelectedItem()` invalidates the current attempt. |
| LookUpGrid | `src/components/LookUpGrid/*` | Resolution/search diagnostics, localized resolution failure, `dataVersion`, selected-item refresh, closed-state reload invalidation, IME guards, and clear shortcuts are additive. Local memo invalidation follows semantic context/search/locale/visible-column inputs rather than incidental callback identity. |
| SearchBox | unchanged public contract in `src/components/SearchBox/*` | Browser coverage verifies registered composition events suppress input/Enter searches and completion triggers exactly one search. |

The internal table-style picker is deliberately not exported. SQL/EF/Dapper provider translation, row/column virtualization, conditional-format designers, XLSX appearance export, schema libraries, async validation, and unrelated deferred components remain excluded. No npm dependency was added.

### Phase 24.1 verification

Strict TypeScript, ESLint, all 669 Vitest tests in 67 files, and import-cycle analysis passed; cycle analysis covered 317 source modules. The production library build transformed 319 modules and emitted declarations, source maps, and `dist/cashgear-ui.css`. Storybook 10.5.10 transformed 396 modules. Package verification passed with 198 runtime exports and 1,557 packed files, including declaration, stylesheet, ESM-import, and dry-run tarball checks.

The complete serial Chromium/WebKit semantic sweep passed all 344 interaction and accessibility cases. A subsequently strengthened table-style focus-return assertion exposed WebKit's non-focusing pointer-button behavior; explicit trigger capture corrected it, and the affected WebKit case passed on focused rerun. Firefox was attempted once and remained environment-blocked before page creation by the known 30-second launch timeout: `RenderCompositorSWGL failed mapping default framebuffer`. No Firefox component assertion ran.

All four new `phase-24-1-*` Windows Chromium baselines passed comparison and were visually inspected in light, gallery, dark, and RTL narrow configurations. The complete 279-case Chromium sweep passed 276 cases on its first run. Restoring the unchanged legacy KeyComboBox story copy returned its existing baseline to an exact match; two transient pre-existing Map tile-position captures also passed unchanged on immediate focused rerun. No pre-existing baseline file was modified.

## Phase 25 — CgPdfViewer

Reference audit: Phase 25 mirrors the clean `CashGear.Blazor.UI/Components/Documents/PdfViewer/*` subtree at commit `32edd96e1220ec5cf1b5ed691795fd95f605c19a`. The Blazor repository remained read only. The React port retains the source viewer's navigation, zoom, rotation, search, thumbnails, outline, password, print, download, fullscreen, external-link, form-field, loading, empty, error, lifecycle, localization, and template behavior.

| Area | React implementation | Source-aligned behavior and adaptation |
| --- | --- | --- |
| Source and lifecycle | `src/components/PdfViewer/CgPdfViewer.tsx`, `CgPdfViewer.types.ts`, `model.ts` | A discriminated URL/data/provider source replaces .NET URL/stream loading. Providers return `Response`, `Blob`, `ArrayBuffer`, or `Uint8Array` and receive a monotonic request ID plus generation-scoped `AbortSignal`; replacement and unmount abort stale work. Controlled/default page, zoom, zoom mode, rotation, sidebar, and search state remain authoritative. |
| Browser engine | `src/components/PdfViewer/pdfController.js` | The controller keeps one PDF.js event bus/viewer instance per React component, lazy-loads the pinned runtime, renders pages on demand, and reports typed state/lifecycle details. React actions cover load/reload, page movement, zoom, rotation, search, print, download, fullscreen, password submission, and metadata. |
| Security | `src/components/PdfViewer/model.ts`, `pdfController.js` | HTTP(S) document URLs are validated; provider authorization remains host-owned. JavaScript actions, XFA, dynamic evaluation, and the scripting sandbox are disabled or absent. External HTTP(S)/mail/tel links and AcroForm controls are independently opt-in, external navigation is host-cancellable, passwords never enter lifecycle details, and default limits reject oversized or excessively long documents. |
| Runtime assets | `vendor-src/pdfjs/*`, `src/vendor/pdfjs/*`, `vite.config.ts` | `pdfjs-dist` 6.2.108 is reproducibly selected by an exact lockfile. Only browser runtime, viewer, CMap, font, ICC, image, and non-eval codec assets are retained. SHA-256 coverage includes every shipped file except the manifest itself; Vite copies those bytes to `dist/vendor/pdfjs`, and `assetBaseUrl` defaults to `/cashgear-ui/pdfjs/`. |
| Evidence | `tests/pdf-viewer-model.test.ts`, `tests/pdf-viewer.test.tsx`, `tests/browser/phase25.*`, `public/pdf-samples/*` | Unit tests use a controlled adapter for cancellation and controlled-state boundaries. Chromium/WebKit tests use the real PDF.js worker against deterministic 48-page, AcroForm/link, and encrypted fixtures. Stories cover interactive, password, security, provider, custom-state, dark, RTL, and narrow layouts. |

Supporting APIs remain type-only exports; `CgPdfViewer` is the sole new root runtime export. The PDF controller, validation/loading model, and PDF.js modules are private implementation details. Consumers must deploy the emitted PDF.js directory unchanged, and application authorization remains outside the component.

### Phase 25 verification

Final verification on 2026-09-06 passed strict TypeScript and ESLint, all 683 Vitest tests in 69 files, and import-cycle analysis across 321 source modules. The production library build transformed 324 modules and emitted the component as `dist/components/PdfViewer/CgPdfViewer.js`, alongside declarations, maps, styles, and unchanged PDF.js assets. Storybook transformed 401 modules. Package verification passed with 199 runtime exports and 2,159 packed files, including exact PDF.js manifest coverage, byte-for-byte `dist` comparison, ESM import, declaration/stylesheet, and dry-run tarball checks.
